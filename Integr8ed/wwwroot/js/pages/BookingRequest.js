...
var table = {
    id: "",
    dtPenPharmacy: "#dtPendingRequest",
    dtAccPharmacy: "#dtAcceptedRequest",
    dtRejPharmacy: "#dtRejectedRequest",

    StatusAddress: "/ClientAdmin/BookingRequest/ManageManageRequest",
};
var custom = {
...
function SetPharmacyStatus(bookingId, status) {
    RxFair.Confirm("Change Pharmacy Status", RxFair.typeModel.Warning, "Are you sure?", RxFair.typeModel.Danger, "Yes, change it!", function () {
        $.ajax({
            url: table.StatusAddress + "?id=" + bookingId + "&status=" + status,
            type: 'POST',
            success: function (response) {
                $(table.id).DataTable().row("[data-RowId='" + bookingId + "']").remove().draw(false);
